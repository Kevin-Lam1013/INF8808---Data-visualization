...
function getTopPlayers(data) {
  ...
  var count = data.reduce(function (res, item) {
    if (res[item.Player]) {
      res[item.Player]++;
    } else {
      res[item.Player] = 1;
  ...
  var topPlayers = Object.entries(count).sort(function (a, b) {
    return b[1] - a[1];
  }).map(function (v) {
    return v[0];
  });
...
function summarizeLines(data) {
  ...
  var nestedData = data.reduce(function (res, item) {
    var key = item.Act;
    ...
    if (!res[key]) {
      ...
      var isIn = false;
      res[key].Players.forEach(function (player) {
...
function replaceOthers(data, top) {
  ...
  var newData = data.reduce(function (res, item) {
    var key = item.Act;
    res[key] = {
      Act: key,
      Players: []
    ...
    item.Players.forEach(function (player) {
      var isTopFive = top.indexOf(player.Player) > -1;

      if (isTopFive) {
        res[key].Players.push(player);
      } else {
        var isIn = false;
        res[key].Players.forEach(function (item) {
...
function updateGroupXScale(scale, data, width) {
  ...
  var groups = d3.map(data, function (group) {
    return group.Act;
  }).keys();
...
function createGroups(data, x) {
  ...
  d3.select('#graph-g').selectAll('.group').data(data).join('g').attr('class', 'group').attr('transform', function (item) {
    return 'translate(' + x(item.Act) + ')';
  });
}
/**
  ...
  d3.selectAll('g.group').each(function (group) {
    ...
    try {
      var _loop = function _loop() {
        ...
        target = actData.Players.find(function (x) {
          return x.Player === player;
        });

        if (target) {
          barHeight = target.Count / y.domain()[0] * y.range()[1];
          d3.select(_this).append('rect').attr('x', width * index).attr('y', height - barHeight).attr('height', barHeight).attr('width', width).style('fill', color(player)).on('mouseover', tip.show).on('mouseout', tip.hide).data([{
            Act: actData.Act,
            Player: target
          }]);
        }
...
function draw(data, color) {
  ...
  data.forEach(function (player) {
    var element = d3.select('.legend').append('div').attr('class', 'legend-element');
    element.append('svg').attr('width', legendSquareSize + padding).attr('height', legendSquareSize + padding).append('rect').style('fill', color(player)).attr('width', legendSquareSize).attr('height', legendSquareSize);
...
function getContents(d) {
  ...
  */
  var toolTip = d3.create();
  toolTip.append('span').attr('id', 'tooltip-title').text('Act ' + d.Act).append('br');
  toolTip.append('br');
  toolTip.append('div').append('label').append('b').text('Player : ').append('text').attr('class', 'tooltip-value').text(d.Player.Player);
  toolTip.append('label').append('b').text('Count : ').append('text').attr('class', 'tooltip-value').text(d.Player.Count);
  return toolTip.html();
}
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "16660" + '/');
