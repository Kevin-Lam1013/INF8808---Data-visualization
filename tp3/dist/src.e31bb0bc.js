...
function getNeighborhoodNames(data) {
  ...
  return _toConsumableArray(new Set(data.map(function (value) {
    return value.Arrond_Nom;
  })));
}
/**
...
function filterYears(data, start, end) {
  ...
  return data.filter(function (value) {
    ...
    return (0, _util.range)(start, end).find(function (e) {
      return e === Number(year);
    });
  });
}
/**
...
function fillMissingData(data, neighborhoods, start, end, range) {
  ...
  var allNeighborhoods = _toConsumableArray(neighborhoods);

  for (var i = 0; i < data.length; i++) {
    if (allNeighborhoods.indexOf(data[i].Arrond_Nom) !== -1) {
      var missingYears = _toConsumableArray(lookForMissingYears(data, data[i].Arrond_Nom, i, start, end, range));
...
function lookForMissingYears(data, neighborhood, index, start, end, range) {
  ...
  var _loop = function _loop(i) {
    ...
    if (allYears.find(function (e) {
      return e === data[i].Plantation_Year;
    })) {
...
function addMissingYears(data, missingYears, neighborhood) {
  _toConsumableArray(missingYears).forEach(function (v) {
    data.push({
...
},{"./util":"scripts/util.js"}],"scripts/viz.js":[function(require,module,exports) {
"use strict";
...
function updateYScale(yScale, neighborhoodNames, height) {
  ...
  yScale.domain(_toConsumableArray(neighborhoodNames).sort()).range([0, height]);
}
/**
...
function drawYAxis(yScale, width) {
  // TODO : Draw Y axis
  d3.select('.y.axis').attr('transform', 'translate(' + width + ', 0)').call(d3.axisRight(yScale));
}
/**
...
function rotateXTicks() {
  ...
  d3.select('.x.axis').selectAll('g').selectAll('text').attr('transform', 'rotate(-45)');
}
/**
...
function updateRects(xScale, yScale, colorScale) {
  ...
  d3.select('#graph-g').selectAll("rect").attr('x', function (d) {
...
function draw(x, y, height, width, fill, colorScale) {
  ...
  d3.select('.legend.bar').attr('x', x).attr('y', y).attr('width', width).attr('height', height).style('fill', fill);
  var legendAxis = d3.select('.legend.axis');
  legendAxis.attr('transform', 'translate(' + x + ',' + y + ')');
  legendAxis.call(d3.axisLeft(d3.scaleLinear().domain(colorScale.domain()).range([height, 0])).ticks(7));
}
...
function setRectHandler(xScale, yScale, rectSelected, rectUnselected, selectTicks, unselectTicks) {
  ...
  d3.selectAll('#graph-g .data-square').on('mouseenter', function () {
...
function rectSelected(element, xScale, yScale) {
  ...
  d3.select(element).append("text") //We need to adjust the xScale and yScale because they give us the coordinates of the top left corner. By adding half of the width and half of the height we get the center coordinates
  .attr('x', xScale(data.Plantation_Year) + boundingBox.width / 2).attr('y', yScale(data.Arrond_Nom) + boundingBox.height / 1.5).attr('text-anchor', 'middle').style('font-size', '14').style('font-weight', 'bold').style('fill', data.Counts < 1000 ? 'black' : 'white').text(data.Counts);
}
/**
...
function selectTicks(name, year) {
  ...
  d3.selectAll("g.axis text")._groups[0].forEach(function (e) {
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "16610" + '/');
