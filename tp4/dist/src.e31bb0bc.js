...
function appendGraphLabels(g) {
  g.append('text').text('CO2 emissions per capita (metric tons)').attr('class', 'y axis-text').attr('transform', 'rotate(-90)').attr('font-size', 12);
  g.append('text').text('GDP per capita ($ USD)').attr('class', 'x axis-text').attr('font-size', 12);
}
/**
...
function drawXAxis(xScale, height) {
  d3.select('.x.axis').attr('transform', 'translate( 0, ' + height + ')').call(d3.axisBottom(xScale).tickSizeOuter(0).tickArguments([5, '~s']));
}
/**
...
},{}],"scripts/scales.js":[function(require,module,exports) {
"use strict";
...
function setRadiusScale(data) {
  ...
  for (var i in data) {
    data[i].forEach(function (country) {
...
function setColorScale(data) {
  ...
  return d3.scaleOrdinal().domain(_toConsumableArray(continents).sort()).range(d3.schemeCategory10);
}
/**
...
function drawCircles(data, rScale, colorScale) {
  ...
  d3.select('#graph-g').selectAll('circle').data(data).enter().append('circle').attr('r', function (d) {
    return rScale(d.Population);
  }).style('fill', function (d) {
    return colorScale(d.Continent);
  }).style('opacity', '70%').style('stroke', 'white');
}
/**
...
function setCircleHoverHandler(tip) {
  ...
  d3.select('#graph-g').selectAll('circle').on('mouseover', function (country, i) {
...
function moveCircles(xScale, yScale, transitionDuration) {
  ...
  d3.select('#graph-g').selectAll('circle').transition().duration(transitionDuration).attr('cx', function (country) {
    return xScale(country.GDP);
  }).attr('cy', function (country) {
    return yScale(country.CO2);
  });
}
/**
...
function setTitleText(year) {
  // TODO : Set the title
  d3.select('#graph-g').selectAll('.title').text('Data for year : ' + year);
}
...
function getContents(d) {
  ...
  var formattedData = [{
    ...
    key: 'GDP : ',
    value: parseFloat(d.GDP).toFixed(2) + ' $ (USD)'
  }, {
    key: 'CO2 emissions : ',
    value: parseFloat(d.CO2).toFixed(2) + ' metric tonnes'
  }];
  var content = d3.create();
  formattedData.forEach(function (info) {
    content.append('div').text(info.key).append('span').text(info.value).attr('class', 'tooltip-value');
  });
  return content.html();
}
...
function drawLegend(colorScale, g, width) {
  ...
  g.append('g').attr('class', 'legend').attr('transform', 'translate(' + width + ', -25)').style('font-size', '15');
  var circleSize = 400;

  var legend = _d3SvgLegend.default.legendColor().title('Legend').scale(colorScale).shape('path', d3.symbol().type(d3.symbolCircle).size(circleSize)());

  g.select('.legend').call(legend);
}
},{"d3-svg-legend":"../node_modules/d3-svg-legend/indexRollupNext.js"}],"../node_modules/d3-selection/src/namespaces.js":[function(require,module,exports) {
"use strict";
...
(function (d3) {
  ...
  d3.json('./countriesData.json').then(function (data) {
...
},{"./scripts/helper.js":"scripts/helper.js","./scripts/scales.js":"scripts/scales.js","./scripts/viz.js":"scripts/viz.js","./scripts/tooltip.js":"scripts/tooltip.js","./scripts/legend.js":"scripts/legend.js","d3-tip":"../node_modules/d3-tip/index.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "3578" + '/');
