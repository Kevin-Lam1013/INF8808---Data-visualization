...
function colorDomain(color, data) {
  ...
  var groups = d3.map(data.features, function (feature) {
...
function mapBackground(data, path, showMapLabel) {
  ...
  d3.select("#map-g").selectAll("path").data(data.features).enter().append("path").attr("d", function (position) {
    return path(position);
  }).attr("stroke", "#ffffff").attr("fill", "#cdd1c4").on("mouseout", function () {
    return d3.select("#marker-g").selectAll("text").remove();
  }).on("mouseover", function (feature) {
    return showMapLabel(feature, path);
  });
}
/**
...
function showMapLabel(d, path) {
  ...
  console.log(path);
  d3.select("#marker-g").append("text").attr("transform", "translate(" + path.centroid(d)[0] + "," + path.centroid(d)[1] + ")").attr("font-size", 10).attr("text-anchor", "middle").text(d.properties.NOM);
}
/**
...
function mapMarkers(data, color, panel) {
  ...
  d3.select("#marker-g").selectAll("circle").data(data.features).enter().append("circle").attr("r", 5).attr("class", "marker").attr("stroke", "white").attr("fill", function (marker) {
    return color(marker.properties.TYPE_SITE_INTERVENTION);
  }).on("click", function (marker) {
    return panel.display(marker, color);
...
function convertCoordinates(data, projection) {
  ...
  */
  data.features.forEach(function (feature) {
    var position = projection(feature.geometry.coordinates);
...
function drawLegend(colorScale, g) {
  ...
  g.append('g').attr('class', 'legend').attr('transform', 'translate(60, 140)').style('font-size', '12');
  var circleSize = 300;

  var legend = _d3SvgLegend.default.legendColor().title('Légende').scale(colorScale).shape('path', d3.symbol().type(d3.symbolCircle).size(circleSize)());
...
function setTitle(g, d, color) {
  // TODO : Set the title
  g.text(d.properties.NOM_PROJET).style('color', color(d.properties.TYPE_SITE_INTERVENTION));
}
/**
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "1082" + '/');
